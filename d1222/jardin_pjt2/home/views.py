...
def prepare_payment(request):
    ...
    data = {
      ...
      "approval_url": "http://127.0.0.1:8000/approve/",
    ...
    print("prepare_payment 전송 데이터:", json.dumps(data, ensure_ascii=False))
    
    response = requests.post(KAKAOPAY_BASE_URL, headers=headers, data=json.dumps(data, ensure_ascii=False))

    result = response.json()
    print("prepare_payment 결과 : ",result)
    print("prepare_payment tid 결과 : ",result["tid"])
...
def kakao_approve(request):
    ...
    print("결과 response : ",response)
    
    result = response.json()
    print("결과창 : ",result.get("aid"))

    # ✅ 결제 성공 여부 체크
    if result.get("aid"):
        # 여기서 DB 저장
        # Payment.objects.create(...)
        Payment.objects.create(member=result['partner_user_id'],tid=tid,order_id=order_id,
            amount=result["amount"]["total"],approved_at=result["approved_at"],  )
